...
SEASTAR_TEST_CASE(test_read_all) {
    return async([] {
        auto check_read_all = [] (input_stream<char>& strm, const char* test) {
            net::packet all = strm.read_all().get0();
            sstring s;
            all.release_into([&s] (temporary_buffer<char> buf) {
        ...
        check_read_all(inp, "aaaaaaaaaaaaaaa");
        ...
        BOOST_REQUIRE_EQUAL(to_sstring(inp_cont.read_all_contiguous().get0()), "aaaaaaaaaaaaaaa");
        BOOST_REQUIRE(inp.eof());
        input_stream<char> inp2_cont(data_source(std::make_unique<test_source_impl>(5, 0)));
        BOOST_REQUIRE_EQUAL(to_sstring(inp2_cont.read_all_contiguous().get0()), "");
    });
}
