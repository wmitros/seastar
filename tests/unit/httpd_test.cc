...
struct echo_stream_handler : public handler_base {
    ...
    future<std::unique_ptr<reply>> handle(const sstring& path,
        ...
        return do_with(std::move(req), std::move(rep), sstring(), [this] (auto& req, auto& rep, auto& rep_content) {
            return do_until([this, &req] { return req->content_stream_ptr->eof(); }, [this, &req, &rep, &rep_content] {
                return req->content_stream_ptr->read().then([this, &rep, &rep_content] (temporary_buffer<char> tmp) {
