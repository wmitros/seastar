...
public:
    ...
    virtual future<temporary_buffer<char>> skip(uint64_t n) override {
        return do_with(temporary_buffer<char>(), [this, &n] (temporary_buffer<char>& buf) {
            return do_until([this, &n] { return n == 0; }, [this, &n, &buf] {
                return this->get().then([this, &n, &buf] (temporary_buffer<char> tmp) {
                    size_t skipping = std::min(n, tmp.size());
                    n -= skipping;
