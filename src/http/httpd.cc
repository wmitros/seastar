...
#include <cctype>
#include <vector>
#include <seastar/http/content_source.hh>
...
static future<std::unique_ptr<httpd::request>>
make_request_content(lw_shared_ptr<input_stream<char>> content_stream, std::unique_ptr<httpd::request> req, bool streaming) {
    if (streaming) {
        req->content_stream = content_stream;
        return make_ready_future<std::unique_ptr<httpd::request>>(std::move(req));
    } else {
        return read_request_body(content_stream, std::move(req));
    }
}
...
future<> connection::read_one() {
    ...
    return _read_buf.consume(_parser).then([this] () mutable {
        ...
        return maybe_reply_continue().then([this] (std::unique_ptr<httpd::request> req) {
            ...
            }).then([this, content_stream] {
                // check if whole content has been read. eof is only set after streams source returns an empty buffer, which might have not happend during handling
                return content_stream->read().discard_result().then([this, content_stream] {
